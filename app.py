...
if not st.session_state.access_granted:
    ...
    st.markdown('<div class="header-container"><div class="mission-header">>> DECRYPTING: PCS MISSION CONTROL ...</div><div class="radar-wrapper"><div class="radar"><div class="radar-grid"></div></div></div></div>', unsafe_allow_html=True)
    ...
    with t1:
        ...
        if save_p and c_name:
            st.toast("📡 Datenübertragung...", icon="⏳")
            ...
            conn.update(worksheet="Profiles", data=pd.concat([df_p, new_p], ignore_index=True))
            st.cache_data.clear()
    ...
    # --- TAB 3: REINIGUNGSMITTE-MODUS (KEIN RANKING) ---
    with t3:
        st.header("Task 3: Ressourcen-Allokation")
        st.write("Verteilen Sie genau 100 Coins auf die identifizierten Sabotage-Risiken.")
        st.markdown("---")
        ...
        if df_coins.empty:
            ...
            for item in df_coins["Thema"].unique():
                ...
                spent += val
            st.markdown(f"### Budget-Status: <span style='color:{'#00FF41' if spent==100 else '#FF4B4B'}; font-weight:bold;'>{spent} / 100 Coins</span>", unsafe_allow_html=True)
            if st.button("FINALIZE TRANSACTION", use_container_width=True, disabled=(spent != 100)):
                st.toast("💰 Transaktion läuft...", icon="⏳")
    ...
    with t4:
        ...
        if df_sabotage_dd.empty:
            st.info("Keine Sabotage-Akten verfügbar.")
        else:
            for idx, row in df_sabotage_dd.iterrows():
                ...
                with st.expander(f"🤿 AKTE: {thema}"):
                    st.write(f"Ursprüngliches Problem: {row['Details']}")
                    diskussion_text = st.text_area("Diskussions-Protokoll:", value=existing_notes, height=150, key=f"dd_text_{thema}")
                    if st.button("💾 NOTIZEN SPEICHERN", key=f"dd_save_{thema}"):
                        st.toast("💾 Sicherung...", icon="⏳")
                        new_dd_row = pd.DataFrame([{"Titel": thema, "Diskussion": diskussion_text}])
                        df_current_dd = get_cached_data("Deep_Dive")
                        if not df_current_dd.empty: df_current_dd = df_current_dd[df_current_dd["Titel"] != thema]
                        conn.update(worksheet="Deep_Dive", data=pd.concat([df_current_dd, new_dd_row], ignore_index=True))
                        st.cache_data.clear()
                        st.success("SAVED")
                        time.sleep(0.5)
                        st.rerun()
